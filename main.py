...
from location import get_current_coordinates
...
cls="w-16 h-16 mx-auto mb-4 "),
                Form(
                    ...
                    Input(
                        ...
                        cls="w-full p-2 rounded mb-7 text-black"
                    ),
                    Button(
                        "Get Weather",
                        type="submit",
                ...
                cls="bg-white/20 backdrop-blur-lg border border-white/30 shadow-xl rounded-2xl p-8 w-80"
            ),
            cls="min-h-screen flex items-center justify-center bg-gradient-to-r from-[#363062] via-[#435585] to-[#818FB4]"
        ),
    )
...
#blocks that check the temperature and display alert messages based on the degree
    if temperature < 0:
        content = (
            ...
            P(dt.datetime.now().strftime("%d.%m.%Y"), cls="text-gray-400 text-center mt-2"),
        )
    elif temperature > 15:
        content = (
...
#if block that checks the temperature and creates a message based on the degree
    if temperature < 0:
        message = "Temperature is under 0 C."
    elif temperature > 15:
        message = "Temperature is over 15 C."
    else:
        message = "Temperature is between 0 C and 15 C."
#creating a dictionary with the city, temperature, date and message
    filtered_response = {
        "city": city,
...
#blocks that check the temperature and display alert messages based on the degree
    if temperature < 0:
        ...
        content = (
            H1(f"Weather in {city}", cls="text-white text-xl text-center mt-4"),
            P(f"Latitude:{latitude}", cls="text-white text-center mt-4"), 
            P(f"Longitude:{longitude}", cls="text-white text-center"),
            P(f"Temperature: {temperature} °C", cls="text-blue-100 text-xl text-center mt-4"),
            P(dt.datetime.now().strftime("%d.%m.%Y"), cls="text-gray-400 text-center mt-2"),
        )
#return that displays form with the weather information, svg icon and button to go back to the home page
    return (
